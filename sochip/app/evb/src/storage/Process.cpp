...
int Process::killProcessesWithOpenFiles(const char *path, int action) {
    ...
    if (!(dir = opendir("/proc"))) {
        db_msg("opendir failed (%s)", strerror(errno));
        return -1;
    }
    ...
    while ((de = readdir(dir))) {
        ...
        if (checkFileDescriptorSymLinks(pid, path, openfile, sizeof(openfile))) {
            db_msg("Process %s (%d) has open file %s", name, pid, openfile);
			if (action == WHETHER_OPENED) {
				return pid;
			}
        } else if (checkFileMaps(pid, path, openfile, sizeof(openfile))) {
            db_msg("Process %s (%d) has open filemap for %s", name, pid, openfile);
        } else if (checkSymLink(pid, path, "cwd")) {
            db_msg("Process %s (%d) has cwd within %s", name, pid, path);
        } else if (checkSymLink(pid, path, "root")) {
            db_msg("Process %s (%d) has chroot within %s", name, pid, path);
        } else if (checkSymLink(pid, path, "exe")) {
            db_msg("Process %s (%d) has executable path within %s", name, pid, path);
        } else {
            continue;
        }
        if (action == 1) {
            db_msg("Sending SIGHUP to process %d", pid);
            kill(pid, SIGTERM);
        } else if (action == 2) {
            db_msg("Sending SIGKILL to process %d", pid);
            kill(pid, SIGKILL);
        }
    }
    closedir(dir);
