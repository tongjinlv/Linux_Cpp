...
static int DialogProc(HWND hDlg, int message, WPARAM wParam, LPARAM lParam)
{
	...
	case MSG_INITDIALOG:
		{
			...
			if(tipLabelData->pLogFont) {
				...
				SetWindowFont(GetDlgItem(hDlg, IDC_TITLE), tipLabelData->pLogFont);
				SetWindowFont(GetDlgItem(hDlg, IDC_TEXT), tipLabelData->pLogFont);
			}
			...
			if(tipLabelData->timeoutMs != TIME_INFINITE) {
				...
				SetTimerEx(hDlg, TIMEOUT_TIMER, tipLabelData->timeoutMs / 25, timerCallback); //ÉèÖÃtimerÎª600ms
			}
		}
		break;
	case MSG_PAINT:
		{
			...
			if (tipLabelData->hasicon == true)
			{
				if(tipLabelData->full_screen != 1){
					#if 0
					char *filepath=(char *)"/etc/res/others/Warning.png";
	...
	case MSG_CLOSE_TIP_LABEL:
		...
		EndDialog(hDlg, 0);
		BroadcastMessage(MSG_RECOVER_ADAS_SCREEN_FLAG,0,0);
		BroadcastMessage(MSG_SHOW_WIFI_INFO,0,0);
		break;
	}
...
int showTipLabel(HWND hParent, tipLabelData_t* info, int full_screen)
{
	...
	dlg.dwStyle = WS_VISIBLE | WS_BORDER;
