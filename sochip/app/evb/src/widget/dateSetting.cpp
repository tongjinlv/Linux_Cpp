...
static int dateSettingProc(HWND hDlg, int message, WPARAM wParam, LPARAM lParam)  
{
	...
	case MSG_KEYDOWN:
		{	
			if(wParam == CDR_KEY_MODE || wParam == CDR_KEY_MENU || wParam == CDR_KEY_OK) {
				tickCnt = GetTickCount();
			}else if(wParam == CDR_KEY_RIGHT){
				SetTimer(hDlg, ID_TIMER_KEY_1, LONG_PRESS_TIME_1);
			}
		}
		break;
	case MSG_TIMER:
		{
			if(wParam == ID_TIMER_KEY_1) {
				SendMessage(hDlg, MSG_KEYUP, CDR_KEY_LEFT,0);
				KillTimer(hDlg, ID_TIMER_KEY_1);
			}
		}
		break;
	case MSG_KEYUP:
		{
			if(wParam == CDR_KEY_RIGHT) {
				if(IsTimerInstalled(hDlg, ID_TIMER_KEY_1)) {	
					KillTimer(hDlg, ID_TIMER_KEY_1);
					db_msg("short press\n");
				}else{
					db_msg("long press\n");
			...
			case CDR_KEY_OK:
				...
				//	db_msg("----------date-time--save over---IDCSelected is: %d \n", IDCSelected);
...
int dateSettingDialog(HWND hParent, dateSettingData_t* configData)
{
	...
	dlgDateSetting.dwStyle = WS_VISIBLE | WS_BORDER;
