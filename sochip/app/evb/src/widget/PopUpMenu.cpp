...
static void paint_content_focus(HDC hdc, PCONTROL pctrl, RECT* prc_cont)
{
	...
	default:
		if(BUTTON_STATUS(pctrl) & BST_FOCUS) {
			text_pixel = DWORD2PIXEL(HDC_SCREEN, 0xFFFFFFFF);
		} else {
			text_pixel = DWORD2PIXEL(HDC_SCREEN, 0xFF1E1E1E);
		}
...
static int button_callback(HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
	...
	case MSG_KEYDOWN:
	case MSG_KEYUP:
		//db_msg("this key event is CDR_KEY_MODE\n");
		return 0;
	default:
		break;
	}
...
static int DialogProc(HWND hDlg, int message, WPARAM wParam, LPARAM lParam)
{
	...
	case MSG_INITDIALOG:
		{
			...
			HWND hChild = GetNextChild(hDlg, 0);
			while( hChild && (hChild != HWND_INVALID) )
			{
				SetWindowFont(hChild, info->pLogFont);
	...
	case MSG_KEYDOWN:
		{
			...
			int id;
			PopUpMenuInfo_t *info;
			SetTimer(hDlg, ID_TIMER1_KEY, LONG_PRESS_TIME);
			key_value = wParam;
	...
	case MSG_KEYUP:
		{
			///db_msg("$$$$$$$$$$$$$$$$$$$$$$ the wParam is %d\n",wParam);
			PCONTROL pCtrl;
			int id;
			PopUpMenuInfo_t *info;
			KillTimer(hDlg, ID_TIMER1_KEY);
			info = (PopUpMenuInfo_t *)GetWindowAdditionalData(hDlg);
			id = GetDlgCtrlID(GetFocusChild(hDlg));
			if(id == -1)
			{
		...
		break;
	case MSG_TIMER:
		SendMessage(hDlg, MSG_KEYLONGPRESS, key_value, 0);
		KillTimer(hDlg, ID_TIMER1_KEY);
		break;
	case MSG_KEYLONGPRESS:
		...
		{
			SendMessage(rm->getHwnd(WINDOWID_MAIN),MSG_SHUT_DOWN,0,0);
		}
		break;
...
int PopUpMenu(HWND hWnd, PopUpMenuInfo_t *info)
{
	...
	for(count = 0; count < info->item_nrs; count++) {
		...
		if(count != 0) {
			//ctrldata[count].dwStyle = WS_VISIBLE | BS_PUSHBUTTON | WS_TABSTOP;
			ctrldata[count].dwStyle = WS_VISIBLE | WS_TABSTOP;
		}
