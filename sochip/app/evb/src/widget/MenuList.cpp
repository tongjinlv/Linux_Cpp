...
static void lstOnDrawSListBoxItems (HWND hWnd, HDC hdc, PLISTBOXDATA pData, int width)
{
	...
	for (i = 0; plbi && i < (pData->itemVisibles + 1); i++) {
		...
		if(plbi->imageFlag & LBIF_IMAGE && plbi->dwImage != 0) {
			...
			value2 = int(pData->itemHeight*0.75);  //30
			//value1 = value2*0.4;  //12
			value1 = value2*0.8;     //24
			
			if(value2 < pData->itemHeight) {
				value3 = (pData->itemHeight - value2) / 2 + y;
			} else 
				value3 =y;
			//db_msg("----------menulist-image-x--y---w---h--==%d  %d  %d  %d \n",x, value3,value1,value2);
		//	FillBoxWithBitmap (hdc, x, value3, value1, value2, (PBITMAP)plbi->dwImage);
			x += ((PBITMAP)(plbi->dwImage))->bmWidth;
		}

		x = 14; /* set the string x start from 18+image width   */
	...
	case MSG_CREATE:
		{
			...
			gp_hilite_bgc	= GetWindowElementPixelEx(hwnd, HDC_SCREEN, WE_BGC_HIGHLIGHT_ITEM);//WE_BGC_HIGHLIGHT_ITEM
			gp_hilite_fgc   = GetWindowElementPixelEx(hwnd, HDC_SCREEN, WE_FGC_HIGHLIGHT_ITEM);
			gp_sign_bgc     = GetWindowElementPixelEx(hwnd, HDC_SCREEN, WE_BGC_SIGNIFICANT_ITEM);//WE_BGC_SIGNIFICANT_ITEM
			...
			/****** scoll bar use the mainc_3dbox element color, the element is RGBA format **********/
			...
			#if 0
			char *filepath=(char *)"/etc/res/others/setup.png";
	...
	case MSG_PAINT:
		{
			...
			if(pData->itemCount == 0){
				...
				int screenW,screenH;
				getScreenInfo(&screenW, &screenH);
				FillBoxWithBitmap(hdc,0,0,screenW,screenH,&bitImage); //luo
			}
			...
			if(pData->itemCount == 0){
				...
				getScreenInfo(&screenW, &screenH);
				FillBoxWithBitmap(hdc,0,0,screenW,screenH,&bitImage); //add by jason
			}
			*/
			GetClientRect (hwnd, &rc);
