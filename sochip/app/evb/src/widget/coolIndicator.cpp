...
static void draw_hilight_box (HWND hWnd, HDC hdc, COOL_INDICATOR_ITEMDATA* item)
{
	...
	SetBrushColor(hdc, item->sliderColor );

	int x, y, w, h;
	x = item->RcTitle.left + 1;
	y = item->RcTitle.bottom + 2;
	w = RECTW(item->RcTitle) - 1;
	h = 2;//4;
	FillBox(hdc, x, y, w, h);
}
    ...
    case MSG_CREATE:
		{
			...
			//db_info("------ItemWidth--is  %d ---ItemHeight is %d\n",TbarData->ItemWidth,TbarData->ItemHeight);
			TbarData->ItemWidth = 28;
			TbarData->ItemHeight = 24;
