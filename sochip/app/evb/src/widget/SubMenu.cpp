	...
	case MSG_INITDIALOG:
		{
            ...
            SetWindowFont(GetDlgItem(hDlg, IDC_SUB_TITLE), subMenuData->pLogFont);
            SetWindowFont(GetDlgItem(hDlg, IDC_SUB_LIST_BOX), subMenuData->pLogFont);

			SetWindowBkColor(hDlg, subMenuData->menuListAttr.normalBgc);
			SetWindowElementAttr(GetDlgItem(hDlg, IDC_SUB_TITLE), WE_FGC_WINDOW, PIXEL2DWORD(HDC_SCREEN, subMenuData->menuListAttr.scrollbarc));
			...
			/* set submenu title */
			SetWindowText(GetDlgItem(hDlg, IDC_SUB_TITLE), subMenuData->title.c_str());
			...
			if(subMenuData->menu_index == subMenuData->menu_tfcard_info)
			{
				...
				for(int i = 0; i < count; i++)
				{
					...
					mlii.string = (char*)subMenuData->contents[i].c_str();
					if(i == 0) 
					{
...
int showSubMenu(HWND hWnd, subMenuData_t* subMenuData)
{
	...
	DlgSubMenu.dwStyle = WS_VISIBLE | WS_CHILD | WS_BORDER;
