...
labelStringCmd_t labelStringCmd[] = {
	...
	{LANG_LABEL_TIPS,				LANG_LABEL_POWER_CHARGING_FULL},
	...
	{LANG_LABEL_TIPS,				LANG_LABEL_TAKE_PICTURE},
};
...
{
	db_msg("<***ShowTipLabel ***>, labelIndex %d--------------timeout:%d--\n", index,timeoutMs);
...
//			index == LABEL_30S_NOWORK_SHUTDOWN ||
	...
	if(index == LABEL_POWER_FULL){
		tipLabelData.timeoutMs = 2 * 1000;
	}
	if (index == LABEL_WIFI_CONNECT) {
		return showTipLabel(hParent, &tipLabelData, 1);
	}
	if(index == LABEL_PHOTO_PROMPT){
		return showTipLabel(hParent, &tipLabelData, 1);
	}
	db_msg("****************ptext:%s",ptext); // null
	if (ptext) {
		static char tipstext[128]={0};
		
		static int my_new_sdy, my_old_sdy = 0;
		my_new_sdy = rm->getResBoolValue(ID_MENU_LIST_DRIVING_MODE);

		enum ResourceID id;
		memset(tipstext, 0, sizeof(tipstext));
		if (index == LABEL_30S_NOWORK_SHUTDOWN) {  
			if((my_new_sdy != my_old_sdy) && (my_new_sdy != 0))
				id = (enum ResourceID)(LANG_LABEL_NOWORK_ENTER_STANDBY);
			else
				id = (enum ResourceID)(LANG_LABEL_30S_NOWORK_SHUTDOWN);
#if 0												//(ID_MENU_LIST_STANDBY_MODE)
			id = (enum ResourceID)(rm->getResBoolValue(ID_MENU_LIST_DRIVING_MODE)  
								?LANG_LABEL_NOWORK_ENTER_STANDBY
								:LANG_LABEL_30S_NOWORK_SHUTDOWN);
#endif
		} else {
			id = (enum ResourceID)LANG_LABEL_CARD_READ_WRITE;
		}
		my_old_sdy = my_new_sdy; //×´Ì¬×ª±ä
		sprintf(tipstext, "%s%s", ptext, rm->getLabel(id));
