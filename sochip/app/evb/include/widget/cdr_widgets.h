...
typedef struct {
	...
	const char* StrProductType;
	const char* StrSoftwareVersion;
	const char* StrUpdated;
...
extern int WifiInfoDialog(HWND hParent, wifi_info_t* configData);
extern int ShowWifiTip(HWND hParent, tipLabelData_t* info, int full_screen);
