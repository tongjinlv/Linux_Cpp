...
static void InitTinyMixer(void)
{
    ...
    retVal = mixer_ctl_set_value(ctl, 0, 100);   // set Line Volume to max
    if (retVal < 0) {
        printf("mixer_ctl_set_value(Spk Volume) failed\n");
    }
...
int main(int argc, char **argv)
{
	...
	char* captureFile = "Rear_Center.wav";
        char* recordFile = "Record.wav";
	if(!strcmp("mic", argv[1])){
		printf("by hero is record mic... start\n");
		pthread_create(&captureTid, NULL, TinyCaptureThread, recordFile);
